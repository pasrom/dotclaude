...
Run `obsidian-init.sh` from the same directory as this SKILL.md:

```bash
bash "$(dirname "$0")/obsidian-init.sh" .
```

The script handles everything: creates `.obsidian/` with app.json (standard markdown links, reading mode), graph.json (color-coded nodes), CSS snippet for status styling, and updates `.gitignore`. After running, commit: `chore(obsidian): initialize vault configuration for viewing`
...
```bash
bash "$(dirname "$0")/obsidian-init.sh" --update .
```

Preserves any user-added files in `.obsidian/`. After running, commit: `chore(obsidian): update vault configuration`
