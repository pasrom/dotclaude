...
## Obsidian (`/km obsidian`)

### `obsidian init`

Sets up the current knowledge base as an Obsidian vault for comfortable viewing and browsing.
This is **view-focused** — it does not change how content is created or linked.

**Prerequisite:** `CONVENTIONS.md` must exist in the repo root. If missing, abort and tell the user to run `/km init` first.

Steps:

1. Create `.obsidian/` directory in the repo root
...
3. Write `.obsidian/appearance.json`:
   ...
   ```
4. Create `.obsidian/snippets/` directory
5. Read `obsidian-km-status.template.css` from the same directory as this SKILL.md and write it to `.obsidian/snippets/km-status.css`
6. Write `.obsidian/graph.json` with color groups for the knowledge base structure:
   ```json
   {
     "colorGroups": [
   ...
   ```
   If a `brains/` directory exists, add an additional color group: `{ "query": "path:brains/", "color": { "a": 1, "rgb": 10092543 } }`
7. Write `.obsidian/community-plugins.json`: `[]`
8. Append Obsidian gitignore block to the repo's `.gitignore` (only if the marker `# Obsidian vault config` is not already present):
   ```
   ...
   ```
9. Inform the user about recommended Obsidian community plugins for the best viewing experience:
   - **Dataview** — query and filter documents by frontmatter (e.g., show all draft decisions)
   - **Frontmatter Tag Suggest** — autocomplete for frontmatter tags
10. If `brains/` exists, note that linked brains appear as subfolders in Obsidian's file explorer and graph view (submodules must be initialized via `git submodule update --init`)
11. Commit: `chore(obsidian): initialize vault configuration for viewing`

### `obsidian update`

Re-generates the Obsidian configuration files. Useful after updating dotclaude or if config was corrupted.

Overwrites: `.obsidian/app.json`, `appearance.json`, `community-plugins.json`, `graph.json`, and `snippets/km-status.css`. Preserves any user-added files in `.obsidian/`.

Follow the same steps as `obsidian init` (skip `.gitignore` append if marker already present). Commit: `chore(obsidian): update vault configuration`
