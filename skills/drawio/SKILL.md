...
name: drawio
description: >-
  Creates diagrams as native draw.io files and optionally exports them to PNG,
  SVG or PDF with the diagram XML embedded, so the exported file stays
  editable in draw.io. Covers flowcharts, architecture, ER, sequence, class
  and network diagrams. Use it when the wanted artifact is a draw.io file or
  an editable diagram export, not when a picture, a sketch or a UI mockup is
  wanted in some other form. Triggers on: draw.io, drawio, .drawio file,
  diagram as draw.io, flowchart, architecture diagram, ER diagram, sequence
  diagram, class diagram, network diagram, export diagram to PNG or SVG,
  Diagramm erstellen, Ablaufdiagramm, Architekturdiagramm, Sequenzdiagramm,
  Diagramm als draw.io, Diagramm exportieren.
---
...
Quote the path: the space in `Program Files` would otherwise split it into two arguments. Single quotes, not backticks, which would run the binary instead of storing its path.
