...
- `skills/` — Claude Code skills (SKILL.md files), symlinked to `~/.claude/skills/` via `install.sh`: git-workflow, comms-archive, datasheet-to-md, cv-extract, ris-search, and session-summarize. The `km` knowledge-management skill lives in its own repository, github.com/pasrom/km.
